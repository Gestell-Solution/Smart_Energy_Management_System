 ...
 * @details    This header file defines macros and constants for configuring
 * Timer1 based on project requirements, including prescaler, waveform mode,
 ...
 * @brief Defines the prescaler for Timer1.
 * @details Sets the clock division factor for Timer1. This affects the timer
 * tick frequency.
 * @note Must be one of the values defined in the Timer1_Prescallers enum.
 */
#define Timer1_Prescaler T1_ClkPrescalling_64
/**
 * @def Timer1_Prescalervalue
 * @brief Defines the prescaler value for Timer1.
 * @details this is used in the equation of getting clk time
 * @note Must be one of the values defined in the Timer1_Prescallers enum.
 */
#define Timer1_Prescalervalue 64
 ...
 * @details In Normal Port mode, the OC1A pin is disconnected and used as a
 * regular I/O.
 */
 ...
 * @details In Normal Port mode, the OC1B pin is disconnected and used as a
 * regular I/O.
 */
 ...
 * @details Sets the base time unit for scheduling tasks, typically in
 * milliseconds.
 */
 ...
 * @details Reads the OCF1A bit from the TIFR register to determine if a compare
 * match occurred.
 ...
 * @details Reads the OCF1B bit from the TIFR register to determine if a compare
 * match occurred.
 * @return Boolean value: 1 if set, 0 if not.
 */
 ...
 */
#define T1_ClkTime ((float)Timer1_Prescalervalue / F_CPU)

/**
 * @def T1_NUMBER_OF_CTC
 * @brief Defines the number of CTC channels used in Timer1.
 * @note Adjust this value based on the required number of CTC channels for your
 * application.
 */
 ...
 * @details Determines the value to load into OCR1A to achieve the desired tick
 * resolution.
 ...
 * @details Determines the value to load into OCR1B to achieve the desired tick
 * resolution.
 * @note Depends on clock frequency, prescaler, and desired time resolution.
 * @warning Ensure that the result fits within 16-bit range (0–65535).
 */
...
/**
 * @def Timer1_TOPvalue
 * @brief Defines the TOP value for Timer1 in CTC mode using ICR1.
 *
 * @details This macro sets the value to be loaded into the ICR1 register,
 *          which determines the maximum count value (TOP) for Timer1 in CTC
 *          mode. The timer resets when it reaches this value, creating a
 *          periodic interval.
 *
 * @note The actual timing depends on the system clock frequency and the
 * selected prescaler.
 *
